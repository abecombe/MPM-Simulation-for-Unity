#ifndef CS_SIMULATION_SVD_HLSL
#define CS_SIMULATION_SVD_HLSL
...
void PostProcess(float3x3 B, inout float3x3 U, inout float3x3 V, float3 alpha, float2 beta, inout float3 sigma,
                 float tao)
{
    if (abs(beta[1]) <= tao)
    {
...
{
    return float3(GetBSplineDerivativeHelper(value.x), GetBSplineDerivativeHelper(value.y),
                  GetBSplineDerivativeHelper(value.z));
}


#endif /* CS_SIMULATION_SVD_HLSL */